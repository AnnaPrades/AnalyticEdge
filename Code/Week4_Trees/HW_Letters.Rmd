...
Let's warm up by attempting to predict just whether a letter is B or not.
reate a new variable isB in the dataframe, which takes the value "TRUE" if the observation corresponds to the letter B, and "FALSE" if it does not. You can do this by typing the following command into your R console:
...
Let's consider a baseline method that always predicts the most frequent outcome, which is "not B". What is the accuracy of this baseline method on the test set?
```{r}
table(test$isB)
...
Build a classification tree to predict whether a letter is a B or not, using the training set to build your model. Remember to remove the variable "letter" out of the model, as this is related to what we are trying to predict! 
...
t1 <- table(test$isB, PredictCARTb)
...
set.seed(1000)
isB.Forest = randomForest(isB ~ . -letter, data = train)
#Calculating accuracy
PredictForestB <- predict(isB.Forest, newdata=test, method="class")
t1 <- table(test$isB , PredictForestB)
t1
accuracy <- sum(diag(t1)/margin.table(t1))
accuracy
```
...
train = subset(letters, spl==TRUE)
test = subset(letters, spl==FALSE)
#Baseline accuracy of the testing set
t1 <- table(test$letter)
t1/margin.table(t1) # = a t1/nrow(test)
max(t1/margin.table(t1)) # 0.2573813
```
...
t1 <- table(test$letter, predictCARTletter)
accuracy <- sum(diag(t1))/margin.table(t1)
accuracy #0.8786906
```
...
#Calculating accuracy
pred.letterForest <- predict(letterForest, newdata=test, method="class")
t1 <- table(test$letter , pred.letterForest)
t1
accuracy <- sum(diag(t1)/margin.table(t1))
accuracy #0.9801027
```
